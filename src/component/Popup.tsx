...
const TabItem = React.memo(({ tab, onRemove }: { tab: Tab; onRemove: (id: number) => void }) => {
...
export default function Popup() {
  ...
  useEffect(() => {
    chrome.storage.sync.get("tabs", (data) => {
      ...
      const updatedStoredTabs = storedTabs.map((tab: Tab) => ({
        ...tab,
  ...
  return (
    ...
    >
      ...
      <div className="mb-6">
        ...
        <div
          className={`space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar ${isScrolling ? "scrolling" : ""}`}
          onScroll={handleScroll}
        >
          <AnimatePresence>
            {inactiveTabs.length > 0 ? (
              inactiveTabs.map((tab) => <TabItem key={tab.id} tab={tab} onRemove={removeTab} />)
            ) : (
              <motion.div
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
